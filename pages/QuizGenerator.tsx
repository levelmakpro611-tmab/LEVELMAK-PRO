  ...
  return (
    <div className="max-w-6xl mx-auto py-4 md:py-8 space-y-8 md:space-y-12 px-4 md:px-0 pt-20 md:pt-8 animate-fade-in">
      ...
      >
        ...
        <div className="inline-flex w-fit self-start md:self-auto bg-slate-100 dark:bg-white/5 p-1 rounded-2xl border border-slate-200 dark:border-white/10 shrink-0">
          <button
            onClick={() => setViewMode('generator')}
            className={`px-4 py-2.5 rounded-xl text-[10px] md:text-xs font-black uppercase tracking-widest transition-all ${viewMode === 'generator' ? 'bg-primary text-white shadow-glow' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-200 dark:hover:bg-white/5'}`}
          >
            {t('quiz.generator.tabs.new')}
          </button>
          <button
            onClick={() => setViewMode('saved')}
            className={`px-4 py-2.5 rounded-xl text-[10px] md:text-xs font-black uppercase tracking-widest transition-all ${viewMode === 'saved' ? 'bg-primary text-white shadow-glow' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-200 dark:hover:bg-white/5'}`}
          >
      ...
      {viewMode === 'saved' ? (
        <div className="space-y-8 animate-fade-in">
          ...
          {quizzes.length === 0 ? (
            <div className="py-20 text-center space-y-4">
              <div className="w-20 h-20 bg-slate-100 dark:bg-white/5 rounded-3xl flex items-center justify-center mx-auto text-slate-400 dark:text-slate-600">
                <BookOpen size={40} />
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {quizzes.map((quiz) => {
                ...
                return (
                  <div
                    key={quiz.id}
                    className={`bg-white dark:bg-slate-900/60 p-6 rounded-[2rem] border border-slate-200 dark:border-white/5 shadow-xl ${theme.hoverBorder} transition-all group relative overflow-hidden flex flex-col`}
                  >
                    ...
                    <div className="relative z-10 space-y-4 flex-1 flex flex-col pt-4">
                      <div className="flex-grow">
                        ...
                        <p className="text-[10px] text-slate-500 font-bold">{quiz.questions.length} Questions</p>
                      </div>
                      ...
                      <div className="flex items-center gap-2 pt-3 border-t border-slate-200 dark:border-white/5">
                        ...
                        <button
                          ...
                          className="w-12 h-12 flex items-center justify-center bg-slate-100 dark:bg-white/5 text-slate-500 hover:text-red-500 hover:bg-red-500/10 rounded-xl transition-all border border-slate-200 dark:border-white/5"
                        >
                          <Trash2 size={16} />
                        </button>
                      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-12 animate-fade-in">
          <div className="lg:col-span-8 space-y-8">
            ...
            <motion.div
              layout
              className="bg-white dark:bg-slate-900/40 backdrop-blur-3xl rounded-[2.5rem] md:rounded-[3.5rem] border border-slate-200 dark:border-white/5 p-6 md:p-10 shadow-xl min-h-[400px] md:min-h-[550px] flex flex-col relative group"
            >
              <AnimatePresence mode="wait">
                {isProcessing ? (
                  ...
                  >
                    ...
                    <h3 className="text-xl md:text-2xl font-black text-slate-900 dark:text-white">{status}</h3>
                  </motion.div>
                ) : inputMode === 'file' ? (
                  <motion.div key="file-mode" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex-1 flex flex-col space-y-6">
                    ...
                    {isCameraActive ? (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        ...
                        <button
                          ...
                          className="w-full flex items-center justify-center gap-3 py-4 md:py-5 bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 text-slate-800 dark:text-white rounded-2xl md:rounded-3xl font-black uppercase tracking-widest text-xs md:text-sm border border-slate-200 dark:border-white/5 active:scale-95 transition-all shadow-sm"
                        >
                      ...
                      <AnimatePresence>
                        {files.map((f) => (
                          <motion.div
                            ...
                            className="relative aspect-square rounded-2xl md:rounded-3xl border border-slate-200 dark:border-white/10 overflow-hidden group/item bg-slate-100 dark:bg-white/5"
                          >
                    ...
                    <p className="text-center text-[10px] md:text-xs text-slate-600 dark:text-slate-400 font-bold italic">
                  ...
                  <motion.div key="text-mode" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex-1 flex flex-col">
                    <textarea
                      ...
                      className="flex-1 w-full p-6 md:p-10 bg-slate-100 dark:bg-white/5 rounded-[2.5rem] border border-slate-200 dark:border-white/5 text-slate-900 dark:text-white font-medium text-lg leading-relaxed outline-none resize-none placeholder:text-slate-400"
                    />
                  </motion.div>
                )}
              </AnimatePresence>
          ...
          <div className="lg:col-span-4 space-y-8">
            <div className="bg-white dark:bg-slate-900/80 rounded-[2.5rem] border border-slate-200 dark:border-white/5 p-6 md:p-8 space-y-8 sticky top-8 shadow-xl">
              <div className="space-y-6">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-700 dark:text-slate-400 px-1">{t('quiz.generator.intensity')}</label>
                <div className="grid grid-cols-1 gap-3">
                  {['Facile', 'Intermédiaire', 'Expert'].map(d => (
                    <button
                      key={d}
                      onClick={() => setDifficulty(d)}
                      className={`p-4 rounded-2xl font-bold border transition-all ${difficulty === d ? 'border-primary bg-primary/10 dark:bg-primary/20 text-primary dark:text-white shadow-glow' : 'border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-transparent text-slate-700 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-white/5'}`}
                    >
              ...
              {error && (
                <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-2xl flex gap-3 text-red-600 dark:text-danger text-[10px] font-bold">
                  <Info size={16} /> {error}
                </div>
              )}
            </div>
