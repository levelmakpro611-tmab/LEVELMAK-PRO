...
const NotificationsManager: React.FC<NotificationsManagerProps> = ({ onNavigate }) => {
    ...
    return (
        <div className="space-y-6 flex flex-col" style={{ minHeight: 'calc(100vh - 200px)' }}>
            {/* Header controls */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 items-center print:hidden bg-white/5 backdrop-blur-xl p-4 rounded-2xl border border-white/10">
                {/* Search */}
                <div className="relative">
                    ...
                    <input
                        ...
                        className="w-full bg-slate-950/50 border border-white/10 rounded-xl py-2 pl-10 pr-4 text-white placeholder-slate-500 focus:outline-none focus:border-blue-500/50 transition-all text-sm font-medium"
                    />
                </div>
                ...
                <div className="flex gap-2 overflow-x-auto pb-1 lg:pb-0 no-scrollbar justify-start lg:justify-center">
                    {([
                        ...
                        <button
                            ...
                            className={`px-3 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-all border whitespace-nowrap ${filter === tab.id
                ...
                <div className="flex justify-end gap-2 shrink-0">
                    <button
                        ...
                        className="p-2.5 bg-white/5 hover:bg-white/10 text-slate-400 hover:text-white rounded-xl border border-white/5 transition-all flex items-center justify-center"
                        title="Actualiser"
                        disabled={loading}
                    >
                    ...
                    {unreadCount > 0 && (
                        <button
                            ...
                            className="flex items-center gap-2 px-4 py-2.5 bg-blue-600/10 hover:bg-blue-600/20 border border-blue-500/20 text-blue-400 rounded-xl text-xs font-bold transition-all uppercase tracking-wider"
                        >
                            <CheckCheck size={16} />
                            <span>Tout lire</span>
                        </button>
                    ...
                    {notifications.length > 0 && (
                        <button
                            ...
                            className="flex items-center gap-2 px-4 py-2.5 bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/20 text-rose-500 rounded-xl text-xs font-bold transition-all uppercase tracking-wider"
                        >
                            <Trash2 size={16} />
                            <span>Vider</span>
                        </button>
            ...
            {/* Notification Stream */}
            <div className="flex-1 bg-black/40 backdrop-blur-md rounded-3xl border border-white/10 overflow-hidden flex flex-col shadow-inner shadow-black/50 p-6">
                <div className="flex justify-between items-center mb-6 pb-4 border-b border-white/5">
                    ...
                    <span className="text-xs text-slate-500">{filteredNotifications.length} / {notifications.length} événements</span>
                </div>

                <div className="flex-1 overflow-y-auto space-y-3 pr-2 custom-scrollbar">
                    {filteredNotifications.length === 0 ? (
                        <div className="h-full flex flex-col items-center justify-center text-slate-500 opacity-60 py-24 gap-4">
                            <Bell size={64} className="text-slate-700 animate-bounce duration-1000" />
                            <div className="text-center">
                                <p className="font-bold text-lg text-slate-400">Aucune notification trouvée</p>
                            ...
                            <AnimatePresence initial={false}>
                                {filteredNotifications.map((notif) => (
                                    <motion.div
                                        ...
                                        className={`group relative flex items-start gap-4 p-5 rounded-2xl border transition-all hover:border-white/15 bg-white/5 hover:bg-white/10 ${!notif.read ? 'border-blue-500/20' : 'border-transparent opacity-75'}`}
                                    >
                                        {/* Status badge left */}
                                        {!notif.read && (
                                        ...
                                        <div className="flex-1 min-w-0 self-center">
                                            <div className="flex justify-between items-start gap-4">
                                                ...
                                                <span className={`text-[10px] px-2.5 py-0.5 rounded-full uppercase tracking-wider font-bold border shrink-0 ${getBadgeColor(notif.type)}`}>
                                            ...
                                            <p className="text-sm text-slate-300 mt-1.5 leading-relaxed">{notif.message}</p>
                                            <span className="inline-block text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-2">{timeAgo(notif.timestamp)}</span>
                                        </div>
                                        ...
                                        <div className="flex flex-col md:flex-row items-center gap-2 self-center shrink-0">
                                            {!notif.read && (
                                                <button
                                                    ...
                                                    className="p-2 bg-white/5 hover:bg-blue-500/20 border border-transparent hover:border-blue-500/30 text-slate-400 hover:text-blue-400 rounded-xl transition-all"
                                            ...
                                            {notif.actionTab && (
                                                <button
                                                    ...
                                                    className="p-2 bg-white/5 hover:bg-purple-500/20 border border-transparent hover:border-purple-500/30 text-slate-400 hover:text-purple-400 rounded-xl transition-all"
                                            ...
                                            <button
                                                ...
                                                className="p-2 bg-white/5 hover:bg-rose-500/20 border border-transparent hover:border-rose-500/30 text-slate-400 hover:text-rose-500 rounded-xl transition-all"
                                                title="Supprimer"
                                            >
                                                <Trash2 size={16} />
                                            </button>
